...
theContract :: Contract () GravitySchema ContractError ()
theContract = gravity
...
tests = testGroup "gravity"
    ...
    , goldenPir "test/Spec/gravity.pir" $$(PlutusTx.compile [|| validateGuess ||])

    , HUnit.testCase "script size is reasonable" (reasonable gravityValidator 20000)
    ]
